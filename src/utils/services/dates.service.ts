...
export class DatesHelper {
  ...
  localWeekdayName(timestamp: Date = new Date(this.localNow())): string {
    return this.WEEKDAY_NAMES[timestamp.getDay()]
  }
  ...
  private getDateAlias(
    ...
    const diffDays = Math.floor(
      DateTime.fromJSDate(targetDate).diff(
    ...
    if (diffDays === -1) {
      return 'was yesterday, ' + this.localWeekdayName(targetDate)
    }
  ...
  convertFromZulu(date: Date | string, timezone: string = this.TIMEZONE): Date {
    ...
    return convertedDate.toFormat("MMMM d 'at' HH:mm")
  }
  ...
  toHumanDate(dateInput: string | Date): string {
    ...
    return dt.toFormat("MMMM d 'at' HH:mm")
  }
}
