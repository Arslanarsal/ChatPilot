...
import { GEMINI_MODELS, OPENAI_MODELS, GeminiModelKey, OpenAIModelKey } from 'src/common/logging/getModel';
...
export class AiAssistantConfigService {
    ...
    async getAssistantConfig(contact: Contact): Promise<AssistantConfig | null> {
        ...
        if (!company?.assistant_id) {
            this.logger.error('No assistant_id found for company', { companyId: company?.id });
            return null;
        }
        ...
        try {
            ...
            if (!assistant) {
                this.logger.error('Assistant instructions not found', { assistantId: company.assistant_id });
                return null;
            }
            ...
            return {
                systemPrompt: assistant.system_prompt ?? 'You are a helpful AI assistant.',
    ...
    private getModel(modelName: string): { model: LanguageModel; metadata: { provider: string; model: string } } {
