...
export class FollowUpProcessor extends WorkerHost {
  ...
  getDefaultFollowUpQuery(
    ...
    const query = `SELECT contacts.id,
       ...
       contacts.nr_reminders_sent,
       Greatest(contacts.last_message_received, contacts.last_reminder_sent) AS last_interaction,
              '[Instrução do Sistema] O usuário não respondeu por um tempo. Com base na última interação dele, gere uma mensagem amigável, curta e com formato de call to action necessariamente fazendo uma pergunta que reforce o último call to action que você enviou. Certifique-se de que a mensagem seja curta, em formato de pergunta call to action e e baseada no contexto recente.' AS prompt
FROM   contacts
WHERE
       CASE
  ...
  private bookingReminderConfig(bookingReminderNo: number): {
    ...
    const bookingReminders = {
      1: {
        ...
        Amanhã ${formattedDate} às ${formattedTime} você tem uma consulta agendada aqui na ${contact.companies.name}. 
        
        Posso confirmar a sua presença amanhã?`
        },
        query: `
        SELECT   c.id,
        c.created_at,
      ...
      for (const company of companies) {
        ...
        const contacts = await this.prisma.contacts.findMany({
          where: {
            ...
            is_bot_activated: true,
            archived_on: null,
            OR: [{ lead_status: null }, { lead_status: { notIn: [11] } }], // Not "Inactive" , "unresponsive"
    ...
    try {
      ...
      for (const contact of contacts) {
        try {
          ...
          // Update the contact
          const updateDto =  smartFollowUps.length > 1 ?{ next_smart_follow_up: smartFollowUps[1].scheduled_at, } : { next_smart_follow_up: null,  lead_status: 11,   objection:'Unresponsive Contact'}
          ...
          // If the contact has reached the max follow-ups, set lead_status to Inactive (999)
          if (contact.smart_reminders_sent >= 9) {
            // Assuming 10 total, so after sending the 10th we set inactive
            await this.contactService.updateContact(contact.id, {
              lead_status: 999, // Inactive
            })
          }
